...
(async function main() {
  ...
  const omniVideo = document.getElementById('js-omni-stream')
  const detailedVideo = document.getElementById('js-detailed-stream')
  ...
  const omniPrefix = window.__OMNI_PREFIX__ 
  const detailedPrefix = window.__DETAILED_PREFIX__ 

  // print message on message window
  const println = str => {
  ...
  const peer1 = new Peer(peer_mode);
  const peer2 = new Peer(peer_mode);
  joinTrigger.disabled = "";            // enable joinButtun
  leaveTrigger.disabled = "disabled";   // disable leavebutton

  // on Peer connected
  peer1.on('open', () => {
    println(`Peer Connected (${peer1.id})`)
  })
  peer2.on('open', () => {
    println(`Peer Connected (${peer2.id})`)
  })
  ...
  joinTrigger.addEventListener('click', () => {
    ...
    joinTrigger.disabled = "disabled" // disable joinButton
    leaveTrigger.disabled = ""        // enable leaveButton

    const mode = {mode: getRoomModeByHash()}
    const room1 = peer1.joinRoom(room1Name, mode)
    const room2 = peer2.joinRoom(room2Name, mode)

    room1.once('open', () => {
      println(`=== (${room1Name}) You Joined ===`)
    })

    room2.once('open', () => {
      println(`=== (${room2Name}) You Joined ===`)
    })

    room1.on('peerJoin', peerId => {
      println(`=== (${room1Name}) ${peerId} joined ===`)
    })

    room2.on('peerJoin', peerId => {
      println(`=== (${room2Name}) ${peerId} joined ===`)
    })

    room1.on('stream', async stream => {
      println(`=== (${room1Name}) new stream from ${stream.peerId} ===`)
      if (!omniVideo.srcObject && stream.peerId.indexOf(omniPrefix) == 0) {
        omniVideo.srcObject = stream
        await omniVideo.play().catch(console.error)
      } 
    })
    
    room2.on('stream', async stream => {
      println(`=== (${room2Name}) new stream from ${stream.peerId} ===`)
      if (!detailedVideo.srcObject && stream.peerId.indexOf(detailedPrefix) == 0) {
        detailedVideo.srcObject = stream
        await detailedVideo.play().catch(console.error)
      }
    })

    room1.on('peerLeave', peerId => {
      println(`=== (${room1Name}) peer ${peerId} left ===`)
      if (omniVideo.srcObject && omniVideo.srcObject.peerId == peerId) {
        stopVideo(omniVideo)
      }
    })

    room2.on('peerLeave', peerId => {
      println(`=== (${room2Name}) peer ${peerId} left ===`)
      if (detailedVideo.srcObject && detailedVideo.srcObject.peerId == peerId) {
        stopVideo(detailedVideo)
      }
    })

    room1.once('close', () => {
      println(`=== (${room1Name}) You left ===`)
      stopVideo(omniVideo)
    })

    room2.once('close', () => {
      println(`=== (${room2Name}) You left ===`)
      stopVideo(detailedVideo)
    })
    ...
    leaveTrigger.addEventListener('click', () => {
      room1.close()
      room2.close()
      joinTrigger.disabled = ""           // enable joinButton
      leaveTrigger.disabled = "disabled"  // disable leaveButton
    }, {once: true})
  })
...
//         }
//         if (flipTrigger) {
//           flipTrigger.style.display = "";   // flipTrigger繝懊ち繝ｳ陦ｨ遉ｺ
//         }
...
//           }
//           if (flipTrigger) {
//             flipTrigger.style.display = "none";   // flipTrigger繝懊ち繝ｳ髱櫁｡ｨ遉ｺ
//           }
//           if (muteOn && muteOff) {
